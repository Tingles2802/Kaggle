...
# ls() if unsure (Gotcha #14/15).
#
# Purpose: extend Model 19's eta/depth grid down to max_depth = {2,3} to
# check whether depth=4's grid-edge win continues or reverses. eta fixed
# at 0.1 only (won at depth=4 in 27-07_18; log treats eta as converged,
# interior result) — depth is the axis actually in question.
# Same xgboost 3.x API notes as 27-07_18: use xgb.attr(fit,"best_iteration")
# (0-indexed), evals= not watchlist=.

grid <- expand.grid(eta = 0.1, max_depth = c(2, 3))
...
print(grid)
cat("\nBest combo this script:\n")
print(grid[1, ])
cat("\nvs Model 19 (eta=0.1, depth=4): 1.176656\n")
